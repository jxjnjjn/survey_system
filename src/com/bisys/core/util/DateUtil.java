...
public class DateUtil {
	...
	public static Map<String, String> getFirstday_Lastday_Month(String date) {
		SimpleDateFormat dfTemp = null;
		if(6 == date.length()){
			dfTemp = new SimpleDateFormat("yyyyMM");
		}else{
			dfTemp = new SimpleDateFormat("yyyyMMdd");
		}
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
        
        Date theDate = null;
        try {
        	theDate = dfTemp.parse(date);
		} catch (ParseException e) {
			e.printStackTrace();
		}
        Calendar calendar = Calendar.getInstance();
	...
	public static List<String> getDateList(String startdate, String enddate){
		...
		Date startDay = StringToDate(startdate, "yyyy-MM-dd");
		Date endDay = StringToDate(enddate, "yyyy-MM-dd");
		for(Date temp = startDay; temp.getTime() <= endDay.getTime(); temp = addDate(temp, 1)){
			result.add(dateToString(temp, "yyyy-MM-dd"));
		}
		return result;
	}
}
